...
gb_internal String mc_concat(String a, String b, String c, String d, String e) {
	return concatenate5_strings(mc_allocator, a, b, c, d, e);
}

gb_internal String mc_concat(String a, String b, String c, String d, String e, String f) {
	return concatenate6_strings(mc_allocator, a, b, c, d, e, f);
}
...
gb_internal void find_windows_kit_paths(Find_Result *result) {
	...
	if (windows10_root.len) {
		...
		if (data_lib.best_name.len && data_bin.best_name.len) {
			...
			if (target_arch.len != 0 && host_arch.len != 0) {
				...
				result->windows_sdk_bin_path          = mc_concat(data_bin.best_name, str_lit("\\"), host_arch, str_lit("\\"));
				sdk_found = true;
			}
		}
	}
...
gb_internal void find_windows_kit_paths_from_env_vars(Find_Result *result) {
	...
	if (win_sdk_ver_bin_path_env.len || ((win_sdk_bin_path_env.len || win_sdk_dir_env.len || crt_sdk_dir_env.len) && (win_sdk_ver_env.len || win_sdk_lib_ver_env.len))) {
		...
		result->windows_sdk_bin_path = mc_concat(bin, host_arch, str_lit("\\"));
		sdk_bin_found = true;
	}
