...
WINDOWS_ARM64_RSA_PSS_OPT_BUG :: ODIN_OS == .Windows &&
	ODIN_ARCH == .arm64 &&
	ODIN_OPTIMIZATION_MODE != .None &&
	ODIN_OPTIMIZATION_MODE != .Minimal

// BUG/LehMichael: LLVM miscompiles pss_sig_unpad on optimized Windows ARM64 builds,
// producing a nonesensical bounds-check failure.
// This seems to be an LLVM Bug, because on 22.1.8 the test passes as expected.
// Probably related to the RISC-V issue with the same test.
when !WINDOWS_ARM64_RSA_PSS_OPT_BUG {
	@(test)
